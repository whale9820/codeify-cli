...
const packages = [
	{ directory: "packages/ai", name: "codeify-ai" },
	{ directory: "packages/tui", name: "codeify-tui" },
	{ directory: "packages/agent", name: "codeify-agent-core" },
	{ directory: "packages/storage/sqlite-node", name: "codeify-storage-sqlite-node" },
	{ directory: "packages/coding-agent", name: "codeify-coding-agent" },
];
...
function prepareOutputDirectory(options, repoRoot) {
	if (!options.outDir) {
		return mkdtempSync(join(tmpdir(), "codeify-local-release-"));
	}
...
function buildBunBinaryRelease(targetDirectory, archiveDirectory) {
	...
	const archiveName = platform.startsWith("windows-") ? `codeify-${platform}.zip` : `codeify-${platform}.tar.gz`;
	cpSync(join(binaryBuildDirectory, archiveName), join(archiveDirectory, archiveName));
	return platform;
}

function createCodeifyShim(installDirectory) {
	const binDirectory = join(installDirectory, "node_modules", ".bin");
	if (process.platform === "win32") {
		if (existsSync(join(binDirectory, "codeify.cmd"))) {
			writeFileSync(join(installDirectory, "codeify.cmd"), '@ECHO off\r\n"%~dp0node_modules\\.bin\\codeify.cmd" %*\r\n');
			writeFileSync(join(installDirectory, "codeify.ps1"), '& "$PSScriptRoot/node_modules/.bin/codeify.ps1" @args\n');
			return;
		}
		writeFileSync(join(installDirectory, "codeify.cmd"), '@ECHO off\r\n"%~dp0node_modules\\.bin\\codeify.exe" %*\r\n');
		writeFileSync(join(installDirectory, "codeify.ps1"), '& "$PSScriptRoot/node_modules/.bin/codeify.exe" @args\n');
		return;
	}
	symlinkSync(join("node_modules", ".bin", "codeify"), join(installDirectory, "codeify"));
}
...
if (rootPackageJson.name !== "codeify-cli") {
...
if (!options.skipInstall) {
	...
	createCodeifyShim(nodeInstallDirectory);
	...
	if (!options.skipBunInstall) {
		...
		createCodeifyShim(bunInstallDirectory);
	}
}
...
if (!options.skipInstall) {
	...
	console.log(`  ${join(outDir, `codeify-${binaryPlatform}.${String(binaryPlatform).startsWith("windows-") ? "zip" : "tar.gz"}`)}`);
	console.log("\nRun the local Bun binary release from outside the repository:");
	console.log(`  ${join(binaryDirectory, String(binaryPlatform).startsWith("windows-") ? "codeify.exe" : "codeify")} --help`);

	console.log("\nIsolated npm install:");
	console.log(`  ${nodeInstallDirectory}`);
	console.log("\nRun the locally packed npm CLI from outside the repository:");
	console.log(`  ${join(nodeInstallDirectory, process.platform === "win32" ? "codeify.cmd" : "codeify")} --help`);
	...
	if (!options.skipBunInstall) {
		...
		console.log(`  ${join(bunInstallDirectory, process.platform === "win32" ? "codeify.cmd" : "codeify")} --help`);
	}
}
