#!/usr/bin/env bash
#
# Build codeify binaries for all platforms locally.
...
#     codeify-darwin-arm64.tar.gz
#     codeify-darwin-x64.tar.gz
#     codeify-linux-x64.tar.gz
#     codeify-linux-arm64.tar.gz
#     codeify-windows-x64.zip
#     codeify-windows-arm64.zip
...
for platform in "${PLATFORMS[@]}"; do
    ...
    if [[ "$platform" == windows-* ]]; then
        bun build --compile --target=bun-$platform ./dist/bun/cli.js ./src/utils/image-resize-worker.ts --outfile "$OUTPUT_DIR/$platform/codeify.exe"
    else
        bun build --compile --target=bun-$platform ./dist/bun/cli.js ./src/utils/image-resize-worker.ts --outfile "$OUTPUT_DIR/$platform/codeify"
    fi
done
...
for platform in "${PLATFORMS[@]}"; do
    if [[ "$platform" == windows-* ]]; then
        # Windows (zip)
        echo "Creating codeify-$platform.zip..."
        (cd "$platform" && zip -r ../codeify-$platform.zip .)
    else
        # Unix platforms (tar.gz) - use wrapper directory for mise compatibility
        echo "Creating codeify-$platform.tar.gz..."
        mv "$platform" codeify && tar -czf codeify-$platform.tar.gz codeify && mv codeify "$platform"
    fi
done
...
for platform in "${PLATFORMS[@]}"; do
    rm -rf "$platform"
    if [[ "$platform" == windows-* ]]; then
        mkdir -p "$platform" && (cd "$platform" && unzip -q ../codeify-$platform.zip)
    else
        tar -xzf codeify-$platform.tar.gz && mv codeify "$platform"
    fi
done
...
for platform in "${PLATFORMS[@]}"; do
    if [[ "$platform" == windows-* ]]; then
        echo "  $OUTPUT_DIR/$platform/codeify.exe"
    else
        echo "  $OUTPUT_DIR/$platform/codeify"
    fi
done
