...
// Bump this only when generated model metadata requires behavior unavailable in older codeify clients.
const MINIMUM_CODEIFY_VERSION = "0.80.7";
...
function validateIndex(index) {
	...
	for (const catalog of index.catalogs) {
		if (
			...
			typeof catalog.minimumCodeifyVersion !== "string" ||
...
function buildIndex(existingIndex, publication) {
	const entry = {
		minimumCodeifyVersion: MINIMUM_CODEIFY_VERSION,
	...
	const catalogs = (existingIndex?.catalogs || [])
		.filter((catalog) => catalog.minimumCodeifyVersion !== MINIMUM_CODEIFY_VERSION)
		.concat(entry)
		.sort((left, right) => comparePiVersions(left.minimumCodeifyVersion, right.minimumCodeifyVersion));
	return {
...
async function main() {
	...
	const temporaryDir = mkdtempSync(join(tmpdir(), "codeify-model-catalog-"));
	try {
		...
		const currentEntry = currentIndex?.catalogs.find(
			(catalog) => catalog.minimumCodeifyVersion === MINIMUM_CODEIFY_VERSION,
		);
