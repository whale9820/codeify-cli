...
import type { OAuthCredential } from "codeify-ai";
import { readStoredCredential } from "codeify-coding-agent";
...
export function getRadiusUrl(): string {
	return process.env.CODEIFY_RADIUS_URL || DEFAULT_RADIUS_URL;
}

export function getRadiusServerBaseUrl(): string {
	const explicitUrl = process.env.CODEIFY_RADIUS_SERVER_URL;
	if (explicitUrl) {
		return explicitUrl;
	}
