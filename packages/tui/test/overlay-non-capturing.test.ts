...
describe("TUI overlay non-capturing", () => {
	describe("focus management", () => {
		...
		it("microtask-deferred sub-overlay pattern restores focus", async () => {
			const terminal = new VirtualTerminal(80, 24);
			const tui = new TUI(terminal);
			const editor = new FocusableOverlay(["EDITOR"]);
			const timer = new FocusableOverlay(["TIMER"]);
			...
			tui.start();
			try {
				// Simulate a deferred overlay factory that creates a timer synchronously,
				...
				const overlayPromise = new Promise<void>((resolve) => {
					...
					// The callback runs as a microtask after the deferred overlay resolves.
				...
				doneFn();
				// Now await the promise and restore the parent overlay.
