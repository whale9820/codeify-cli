...
describe("CombinedAutocompleteProvider", () => {
	...
	describe("fd @ file suggestions", { skip: !isFdInstalled }, () => {
		...
		test("returns the same @ suggestions when the cwd path contains the query", async () => {
			...
			const structure = {
				dirs: ["packages/coding-agent/examples/plan-mode"],
				files: {
					"packages/coding-agent/examples/plan-mode/README.md": "readme",
			...
			assert.ok(normalize(normalResult).includes("plan-mode/ :: packages/coding-agent/examples/plan-mode"));
