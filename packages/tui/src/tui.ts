...
export class TUI extends Container {
	...
	private showHardwareCursor = process.env.CODEIFY_HARDWARE_CURSOR === "1";
	private clearOnShrink = process.env.CODEIFY_CLEAR_ON_SHRINK === "1"; // Clear empty rows when content shrinks (default: off)
	...
	private doRender(): void {
		...
		const logRedraw = (reason: string): void => {
			...
			const logPath = path.join(os.homedir(), ".codeify", "agent", "codeify-debug.log");
		...
		for (let i = firstChanged; i <= renderEnd; i++) {
			...
			if (!isImage && visibleWidth(line) > width) {
				...
				const crashLogPath = path.join(os.homedir(), ".codeify", "agent", "codeify-crash.log");
				const crashData = [
		...
		if (process.env.CODEIFY_TUI_DEBUG === "1") {
