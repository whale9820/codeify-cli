...
import type { AnthropicMessagesCompat, Api, Context, Model, OpenAICompletionsCompat } from "codeify-ai/compat";
import { getApiProvider, getSupportedThinkingLevels } from "codeify-ai/compat";
import { afterEach, beforeEach, describe, expect, test, vi } from "vitest";
import { AuthStorage } from "../src/core/auth-storage.ts";
import { clearApiKeyCache, type ModelRegistry, type RuntimeProviderConfig } from "../src/core/model-registry.ts";
...
describe("ModelRegistry", () => {
	...
	describe("dynamic provider lifecycle", () => {
		...
		test("modelOverrides apply to dynamically registered provider models", async () => {
			...
			expect(model).toBeDefined();
			if (!model) {
				throw new Error("dynamic model was not registered");
			}
