...
describe("SettingsManager", () => {
	...
	beforeEach(() => {
		...
		mkdirSync(join(projectDir, ".codeify"), { recursive: true });
	});
	...
	describe("preserves externally added settings", () => {
		...
		it("should preserve custom settings when changing theme", async () => {
			...
			currentSettings.customSetting = true;
			writeFileSync(settingsPath, JSON.stringify(currentSettings, null, 2));
			...
			expect(savedSettings.customSetting).toBe(true);
	...
	describe("removed settings", () => {
		it("drops update and analytics settings", () => {
			writeFileSync(
			...
			expect(manager.getGlobalSettings()).toEqual({ theme: "dark" });
		});
	});

	describe("smart model usage", () => {
		it("defaults off and persists explicit changes", async () => {
			const manager = SettingsManager.create(projectDir, agentDir);

			expect(manager.getSmartModelUsage()).toBe(false);
			manager.setSmartModelUsage(true);
			await manager.flush();

			expect(manager.getSmartModelUsage()).toBe(true);
			expect(JSON.parse(readFileSync(join(agentDir, "settings.json"), "utf-8")).smartModelUsage).toBe(true);
		});
	});
	...
	describe("error tracking", () => {
		it("should collect and clear load errors via drainErrors", () => {
			...
			const projectSettingsPath = join(projectDir, ".codeify", "settings.json");
	...
	describe("project trust", () => {
		it("should skip project settings when project is not trusted", () => {
			...
			writeFileSync(join(projectDir, ".codeify", "settings.json"), JSON.stringify({ theme: "project" }));
		...
		it("should fail project settings writes when project is not trusted", async () => {
			const projectSettingsPath = join(projectDir, ".codeify", "settings.json");
			writeFileSync(projectSettingsPath, JSON.stringify({ themes: ["themes/existing"] }));
			const manager = SettingsManager.create(projectDir, agentDir, { projectTrusted: false });

			expect(() => manager.setProjectThemePaths(["themes/new"])).toThrow(
				"Project is not trusted; refusing to write project settings",
			);
			await manager.flush();

			expect(manager.getProjectSettings()).toEqual({});
			expect(JSON.parse(readFileSync(projectSettingsPath, "utf-8"))).toEqual({ themes: ["themes/existing"] });
		});

		it("should read default project trust from global settings only", () => {
			writeFileSync(join(agentDir, "settings.json"), JSON.stringify({ defaultProjectTrust: "always" }));
			writeFileSync(join(projectDir, ".codeify", "settings.json"), JSON.stringify({ defaultProjectTrust: "never" }));
	...
	describe("project settings directory creation", () => {
		it("should not create .codeify folder when only reading project settings", () => {
			// Create agent dir with global settings, but NO .codeify folder in project
			const settingsPath = join(agentDir, "settings.json");
			writeFileSync(settingsPath, JSON.stringify({ theme: "dark" }));

			// Delete the .codeify folder that beforeEach created
			rmSync(join(projectDir, ".codeify"), { recursive: true });

			// Create SettingsManager (reads both global and project settings)
			const manager = SettingsManager.create(projectDir, agentDir);

			// .codeify folder should NOT have been created just from reading
			expect(existsSync(join(projectDir, ".codeify"))).toBe(false);

			// Settings should still be loaded from global
			expect(manager.getTheme()).toBe("dark");
		});

		it("should create .codeify folder when writing project settings", async () => {
			// Create agent dir with global settings, but NO .codeify folder in project
			const settingsPath = join(agentDir, "settings.json");
			writeFileSync(settingsPath, JSON.stringify({ theme: "dark" }));
			...
			// .codeify folder should NOT exist yet
			expect(existsSync(join(projectDir, ".codeify"))).toBe(false);

			// Write a project-specific setting
			manager.setProjectThemePaths(["themes/test"]);
			await manager.flush();

			// Now .codeify folder should exist
			expect(existsSync(join(projectDir, ".codeify"))).toBe(true);

			// And settings file should be created
			expect(existsSync(join(projectDir, ".codeify", "settings.json"))).toBe(true);
		});
	});
	...
	describe("httpIdleTimeoutMs", () => {
		...
		it("should use merged global and project settings", () => {
			...
			writeFileSync(join(projectDir, ".codeify", "settings.json"), JSON.stringify({ httpIdleTimeoutMs: 0 }));
	...
	describe("getSessionDir", () => {
		...
		it("should return project sessionDir, overriding global", () => {
			...
			writeFileSync(join(projectDir, ".codeify", "settings.json"), JSON.stringify({ sessionDir: "./sessions" }));
			const manager = SettingsManager.create(projectDir, agentDir);
