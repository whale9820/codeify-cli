...
describe("ProjectTrustStore", () => {
	...
	it("detects trust-requiring project resources", () => {
		...
		try {
			mkdirSync(join(tempDir, ".codeify", "agent"), { recursive: true });
			...
			writeFileSync(join(tempDir, ".codeify", "settings.json"), "{}");
			expect(hasTrustRequiringProjectResources(tempDir)).toBe(true);
			rmSync(join(tempDir, ".codeify", "settings.json"), { force: true });

			mkdirSync(join(cwd, ".codeify"), { recursive: true });
			writeFileSync(join(cwd, ".codeify", "settings.json"), "{}");
			expect(hasTrustRequiringProjectResources(cwd)).toBe(true);

			rmSync(join(cwd, ".codeify"), { recursive: true, force: true });
