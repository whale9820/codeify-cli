import { type AuthType, type CredentialStore, InMemoryCredentialStore } from "codeify-ai";
...
describe("ModelRuntime auth options", () => {
	...
	it("constructs an API key method for a configured provider", async () => {
		...
		runtime.registerProvider("configured-api-key", {
			...
			apiKey: "$CONFIGURED_TEST_API_KEY",
			api: "openai-completions",
			models: [testModel("configured-model")],
		});

		const options = authOptions(runtime).filter((option) => option.provider.id === "configured-api-key");
		expect(options).toHaveLength(1);
		expect(options[0]).toMatchObject({
			type: "api_key",
			provider: { id: "configured-api-key", name: "Configured API Key" },
			method: { name: "API key" },
		});
	...
	it("does not fabricate an API key method for an OAuth-only provider", async () => {
		...
		const options = authOptions(runtime).filter((option) => option.provider.id === "oauth-only");
		expect(options).toHaveLength(1);
		expect(options[0]).toMatchObject({
			type: "oauth",
			provider: { id: "oauth-only", name: "OAuth-only provider" },
			method: { name: "Provider subscription" },
		});
	});
});
