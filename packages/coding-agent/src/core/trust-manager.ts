...
import { findProjectAgentsSkillsDirs } from "./skills.ts";
 ...
 * project trust: trust-requiring entries under cwd/<config dir>, or
 * .agents/skills in cwd or one of its ancestors up to the git repo root.
...
export function hasTrustRequiringProjectResources(cwd: string): boolean {
	...
	return findProjectAgentsSkillsDirs(currentDir).length > 0;
}
