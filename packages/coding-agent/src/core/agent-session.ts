...
export class AgentSession {
	...
	// Base system prompt used to apply fresh appends each turn
	...
	private async _handlePostAgentRun(): Promise<boolean> {
		...
		// here were queued by agent_end handlers and need a continuation.
		return this.agent.hasQueuedMessages();
	}

	/**
	 * Send a prompt to the agent.
	 * - Expands slash commands immediately, even during streaming
	 ...
	 * Emits errors if file read fails.
	 */
	...
	async sendCustomMessage<T = unknown>(
		...
		const appMessage = {
			...
			// Normalize optional content at ingestion.
	...
	async compact(customInstructions?: string): Promise<CompactionResult> {
		...
		try {
			...
			const { summary, firstKeptEntryId, tokensBefore, usage, details } = result;
	...
	private async _runAutoCompaction(reason: "overflow" | "threshold", willRetry: boolean): Promise<boolean> {
		...
		try {
			...
			const { summary, firstKeptEntryId, tokensBefore, usage, details } = compactResult;
	...
	get smartModelUsageEnabled(): boolean {
		return this.settingsManager.getSmartModelUsage();
	}

	setSmartModelUsageEnabled(enabled: boolean): void {
		this.settingsManager.setSmartModelUsage(enabled);
		const activeToolNames = this.getActiveToolNames().filter((name) => name !== "codeify_model");
		if (enabled) activeToolNames.push("codeify_model");
	...
	private _refreshToolRegistry(options?: { activeToolNames?: string[] }): void {
		...
		const toolContext = (): ToolExecutionContext => ({ cwd: this._cwd, model: this.model });
		const wrappedCustomTools = allCustomTools.map(({ definition }) => wrapToolDefinition(definition, toolContext));
		const wrappedBuiltInTools = Array.from(this._baseToolDefinitions.values())
			.filter((definition) => isAllowedTool(definition.name))
			.map((definition) => wrapToolDefinition(definition, toolContext));
	 ...
	 * Used by executeBash and custom bash operations.
	 */
