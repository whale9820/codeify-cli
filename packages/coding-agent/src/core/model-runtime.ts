...
/** Configured pi-ai Models collection used by Codeify and SDK consumers. */
export class ModelRuntime implements Models {
	private readonly models: MutableModels;
	private readonly credentials: RuntimeCredentials;
	private readonly defaultBuiltins: ReadonlyMap<string, Provider>;
	private readonly builtins = new Map<string, Provider>();
	private readonly nativeProviders = new Map<string, Provider>();
	...
	private recomposeProvider(providerId: string): void {
		const base = this.nativeProviders.get(providerId) ?? this.builtins.get(providerId);
		const configured = this.providerConfigs.get(providerId);
		if (!base && !this.config.getProvider(providerId) && !configured) {
		...
		try {
			this.models.setProvider(composeModelProvider(providerId, base, this.config, configured));
			this.compositionErrors.delete(providerId);
		} catch (error) {
	...
	getRegisteredProviderConfig(providerId: string): RuntimeProviderConfig | undefined {
		return this.providerConfigs.get(providerId);
	}

	getRegisteredProviderIds(): readonly string[] {
		return [...new Set([...this.providerConfigs.keys(), ...this.nativeProviders.keys()])];
	}

	getRegisteredNativeProvider(providerId: string): Provider | undefined {
		return this.nativeProviders.get(providerId);
	}
	...
	getCompatibilityRequestConfig(model: Model<Api>): CompatibilityRequestConfig {
		return resolveCompatibilityRequestConfig(
			...
			this.providerConfigs.get(model.provider),
		);
	}
	...
	async getAuth(
		...
		const configuredHeaders = resolveConfiguredModelHeaders(
			...
			this.providerConfigs.get(providerOrModel.provider),
	...
	registerNativeProvider(provider: Provider): void {
		...
		this.providerConfigs.delete(provider.id);
	...
	registerProvider(providerId: string, config: RuntimeProviderConfig): void {
		...
		validateProviderConfig(providerId, this.builtins.get(providerId), this.config.getProvider(providerId), config);
		this.nativeProviders.delete(providerId);
		// Re-registration merges defined values over the previous registration and
		// preserves undefined ones, matching the legacy ModelRegistry contract.
		const previous = this.providerConfigs.get(providerId);
		const effective: RuntimeProviderConfig = { ...previous };
		for (const [key, value] of Object.entries(config)) {
			if (value !== undefined) (effective as Record<string, unknown>)[key] = value;
		}
		this.providerConfigs.set(providerId, effective);
		this.recomposeProvider(providerId);
		this.updateModelSnapshot();
		if (
