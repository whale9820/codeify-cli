...
export async function main(args: string[]) {
	...
	const startupBenchmark = isTruthyEnvFlag(process.env.CODEIFY_STARTUP_BENCHMARK);
	if (startupBenchmark && appMode !== "interactive") {
		console.error(chalk.red("Error: CODEIFY_STARTUP_BENCHMARK only supports interactive mode"));
		process.exit(1);
	}
