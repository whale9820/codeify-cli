...
Codeify CLI saves conversations as sessions so you can continue work, branch from earlier turns, and revisit previous paths.

## Session Storage

Sessions auto-save to `~/.codeify/agent/sessions/`, organized by working directory. Each session is a JSONL file with a tree structure.

```bash
codeify -c                  # Continue most recent session
codeify -r                  # Browse and select from past sessions
codeify --no-session        # Ephemeral mode; do not save
codeify --name "my task"    # Set session display name at startup
codeify --session <path|id> # Use a specific session file or partial session ID
codeify --fork <path|id>    # Fork a session file or partial session ID into a new session
```
...
`/resume` opens an interactive session picker for the current project. `codeify -r` opens the same picker at startup.
...
When available, codeify uses the `trash` CLI for deletion instead of permanently removing files.
...
```bash
codeify --name "Refactor auth module"
codeify --name "CI audit" -p "Review this build failure"
```

Named sessions are easier to find in `/resume` and `codeify -r`.
...
When `/tree` switches away from one branch to another, codeify can summarize the abandoned branch and attach that summary at the new position. This preserves important context from the path you left without replaying the whole branch.
...
Session files are JSONL and contain message entries, model changes, thinking-level changes, labels, compactions, and branch summaries.
