> Codeify can create themes. Ask it to build one for your setup.
...
Codeify loads themes from:

- Built-in: `dark`, `light`
- Global: `~/.codeify/agent/themes/*.json`
- Project: `.codeify/themes/*.json` (only after the project is trusted)
...
On first run, Codeify detects your terminal background and defaults to `dark` or `light`.
...
```bash
mkdir -p ~/.codeify/agent/themes
vim ~/.codeify/agent/themes/my-theme.json
```
...
**Hot reload:** When you edit the currently active custom theme file, Codeify reloads it automatically for immediate visual feedback.
...
Codeify uses 24-bit RGB colors. Most modern terminals support this (iTerm2, Kitty, WezTerm, Windows Terminal, VS Code). For older terminals with only 256-color support, Codeify falls back to the nearest approximation.
