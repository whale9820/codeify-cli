...
Stateful agent with tool execution and event streaming. Built on `codeify-ai`.

## Installation

```bash
npm install codeify-agent-core
```

### SQLite session backends

The SQLite session backend and the `node:sqlite` adapter live in a separate package, `codeify-storage-sqlite-node`, so the core package does not pull in runtime builtins or native SQLite dependencies by default. The backend accepts a runtime-specific SQLite factory, allowing other storage backends to ship as their own packages in the future.
...
```typescript
declare module "codeify-agent-core" {
...
```typescript
import { Agent, streamProxy } from "codeify-agent-core";
...
```typescript
import { agentLoop, agentLoopContinue } from "codeify-agent-core";
