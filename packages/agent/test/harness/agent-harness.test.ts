...
describe("AgentHarness", () => {
	...
	it("preserves app tool types for getters and update events", async () => {
		...
		type AppTool = AgentTool<typeof calculateTool.parameters, undefined> & { source: "builtin" | "custom" };
		const inspectTool: AppTool = { ...calculateTool, name: "inspect", source: "builtin" };
		const searchTool: AppTool = { ...calculateTool, name: "search", source: "custom" };
		...
		expect(harness.getTools().map((tool) => tool.source)).toEqual(["custom"]);
