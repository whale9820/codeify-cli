...
import * as oauthTypes from "../src/oauth.ts";
...
describe.sequential("OAuthAuth adapters", () => {
	it("keeps the OAuth barrel free of built-in flow implementations", () => {
		expect(oauthTypes).not.toHaveProperty("loginAnthropic");
		expect(oauthTypes).not.toHaveProperty("anthropicOAuth");
	});
