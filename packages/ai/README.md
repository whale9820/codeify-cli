...
```bash
npm install codeify-ai
```

TypeBox exports are re-exported from `codeify-ai`: `Type`, `Static`, and `TSchema`.
...
```typescript
import { Type, type Context, type Tool } from 'codeify-ai';
import { builtinModels } from 'codeify-ai/providers/all';
...
import { openaiProvider } from 'codeify-ai/providers/openai';
import { openrouterProvider } from 'codeify-ai/providers/openrouter';
import { amazonBedrockProvider } from 'codeify-ai/providers/amazon-bedrock';
...
```typescript
import { getBuiltinModel, getBuiltinModels, getBuiltinProviders } from 'codeify-ai/providers/all';
...
```typescript
import { createModels, type CredentialStore } from 'codeify-ai';
...
```typescript
import { Type, type Tool, StringEnum } from 'codeify-ai';
...
```typescript
import { validateToolCall, type Tool } from 'codeify-ai';
...
```typescript
import { builtinImagesModels } from 'codeify-ai/providers/all';
...
```typescript
import { getImageModel, generateImages } from 'codeify-ai/compat';
...
```typescript
import { createModels, createProvider, envApiKeyAuth, type Model } from 'codeify-ai';
import { openAICompletionsApi } from 'codeify-ai/api/openai-completions.lazy';
...
```typescript
import { anthropicMessagesApi } from 'codeify-ai/api/anthropic-messages.lazy';
import { openAIResponsesApi } from 'codeify-ai/api/openai-responses.lazy';
...
```typescript
import { stream } from 'codeify-ai/api/anthropic-messages';
...
Importing an implementation module loads its SDK. The `./api/<id>.lazy` wrappers (used by the provider factories) defer that load to the first request when the runtime or bundler supports dynamic import chunking. Legacy raw API subpaths from older releases (`./anthropic`, `./google`, `./mistral`, `./openai-completions`, ...) were removed; use `codeify-ai/api/<api-id>`.
...
```typescript
import { createModels, type Context } from 'codeify-ai';
import { anthropicProvider } from 'codeify-ai/providers/anthropic';
import { openaiProvider } from 'codeify-ai/providers/openai';
import { googleProvider } from 'codeify-ai/providers/google';
...
- `codeify-ai` is the core entrypoint and does not import built-in catalogs, provider factories, or SDK implementations.
- `codeify-ai/providers/<provider>` imports that provider's catalog and lazy API wrapper only.
- `codeify-ai/providers/all` imports every built-in provider factory and all catalogs. Use it only when you want the full built-in set.
...
- Importing `codeify-ai/api/<api-id>` directly loads that API implementation and its SDK immediately.

Avoid `codeify-ai/compat` in new bundled apps; it preserves the old global API and imports the full built-in catalog surface.
...
```typescript
import { setBedrockProviderModule } from 'codeify-ai/api/bedrock-converse-stream.lazy';
import { bedrockProviderModule } from 'codeify-ai/bedrock-provider';
...
Pass `env` in stream options to scope provider configuration to a request. Values in `env` are used before process environment variables for provider auth and configuration such as Cloudflare account IDs, Azure OpenAI settings, Vertex project/location, Bedrock settings, `CODEIFY_CACHE_RETENTION`, and `HTTP_PROXY`/`HTTPS_PROXY`.
...
```bash
npx codeify-ai login              # interactive provider selection
npx codeify-ai login anthropic    # login to specific provider
npx codeify-ai list               # list available providers
```
...
Built-in login and refresh flows are private provider implementations. Use provider-owned `OAuthAuth`, which composes with `CredentialStore` and gets locked auto-refresh through `Models`. The `codeify-ai/oauth` entry point retains compatibility type declarations used by the coding agent.
...
```typescript
// Before
import { getModel, complete } from 'codeify-ai';

// After (verbatim behavior, one import-path change)
import { getModel, complete } from 'codeify-ai/compat';
```
...
| `streamAnthropic(model, ctx, opts)` | `stream` from `codeify-ai/api/anthropic-messages`, or a provider in a collection |
...
Add a lazy wrapper `src/api/<api-id>.lazy.ts` (`<name>Api()` via `lazyApi()`) so providers can reference the implementation without importing its SDK. Add any root-level `export type` re-exports in `src/index.ts` that should remain available from `codeify-ai`.
